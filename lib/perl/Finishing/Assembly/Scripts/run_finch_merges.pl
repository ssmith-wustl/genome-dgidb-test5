...
if ($queue and !$on_queue){
    exec ("bsub -q long -J RUNMERGES -g /finfo -oo $base_dir/$logfile -u adukes perl -I /gscuser/adukes/svn/test_modules $script_path @args --on-queue");
}
...
sub create_job{
    ...
    #($bac) = $bac =~ /^([^\.]+)\./;
    my $bac = $ops_hash{'bac'};
    my ($acefile) = grep { $_ =~ /ace\.0$/} Finishing::Assembly::Ace::Dir->new(dir => $ENV{SEQMGR}."/$bac/edit_dir")->acefiles;
    ...
    system "cp $file_path $result_dir";
    my $pp = undef;
...
if ($left_ctg_num and !($left_ctg_num eq 'NA')){
...
if ($right_ctg_num and !($right_ctg_num eq 'NA')){
...
if ($left_contig){
    main->info_msg("splitting left contig @ $left_end_pos.  Contig length = ".$left_contig->length);
    ($left_merge_contig,my $dump) = $ct->split($left_contig,undef, no_gui=>1, split_position => $left_end_pos, phd_array => [$po,$phddb], allow_no_split => 1);
    main->info_msg("split! left contig length = ".$left_contig->length.", discarded right side length = ".$dump->length);
    main->info_msg('done left');
}
if ($right_contig){
    main->info_msg("splitting left contig @ $right_end_pos.  Contig length = ".$right_contig->length);
    (my $dump,$right_merge_contig) = $ct->split($right_contig,undef, no_gui=>1, split_position => $right_end_pos, phd_array => [$po,$phddb], allow_no_split => 1);
    main->info_msg("split! right contig length = ".$right_contig->length.", discarded left side length = ".$dump->length);
