...
# This version has been modified to work better with the touch screen application

package TouchScreen::BrowseEntry;
...
sub Populate {
    ...
    $c->withdraw;
    my $sl = $c->Scrolled( qw/Listbox -selectmode browse -scrollbars oe -font '-*-times-medium-r-*-*-4-*-*-*-*-*-*-*' -height 5/ );
...
sub SetBindings {
    ...
    # set bind tags
    $w->bindtags([$w, 'Tk::BrowseEntryHack', $w->toplevel, 'all']);
...
sub LbChoose {
	...
	# HACK TO WORK WITH HLIST
	my $var;
	if(ref $browsecmd eq "Tk::Callback") {
		$var = $browsecmd->[1];
		$browsecmd = $browsecmd->[0];
	}	

	if (defined $browsecmd) {
	    &$browsecmd($w, $w->Subwidget('entry')->get(), $var);
	}
    }
}
...
sub LbIndex {
    ...
    if (defined $sel) {
	return int(ref($sel) eq "ARRAY" ? $sel->[0] : $sel);
    } else {
