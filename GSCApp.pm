...
BEGIN {
    if ($^O eq 'MSWin32' || $^O eq 'cygwin') {
        ...
        if ($^O eq 'cygwin') {
            system("echo Perl $]: $0 >> //winsvr/var/log/confirm_scheduled_pse/test/windows.txt");
        }
        else {
...
App::Vocabulary->vocabulary(qw/GSC DNA NCBI FPC ID ABI SCF GB PSE MD PCR cDNA mRNA tRNA QStat ZTR NIH NHGRI MP FTP IPR HTMP QA AB1 ETL OLTP OLAP NSF CDS UTR SFF TA TCGA RNA SRA/);
